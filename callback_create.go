...
func createCallback(scope *Scope) {
	if !scope.HasError() {
		...
		if lastInsertIDReturningSuffix == "" || primaryField == nil {
			if result, err := scope.SQLDB().ExecContext(context.TODO(), scope.SQL, scope.SQLVars...); scope.Err(err) == nil {
		...
		if primaryField.Field.CanAddr() {
			if err := scope.SQLDB().QueryRowContext(context.TODO(), scope.SQL, scope.SQLVars...).Scan(primaryField.Field.Addr().Interface()); scope.Err(err) == nil {
